...
func defence(	var charName string
, charClass string) string {
	if charClass == "warrior" {
		return fmt.Sprintf("%s блокировал %d урона.", 	var charName string
, 10+randint(5, 10))
	}
	 if charClass == "mage" {
		return fmt.Sprintf("%s блокировал %d урона.", 	var charName string
...
func startTraining(	var charName string
, charClass string) string {
	if charClass == "warrior" {
	...
	for cmd != "skip" {
		...
		if cmd == "attack" {
			fmt.Println(attack(	var charName string
, charClass))
		}

		if cmd == "defence" {
			fmt.Println(defence(	var charName string
, charClass))
		}

		if cmd == "special" {
			fmt.Println(special(	var charName string
, charClass))
		}
	}
...
func choosecCharClass() string {
	var approvChoice string
	...
	for approvChoice != "y" {
		...
		fmt.Scanf("%s\n", &approvChoice)
		approvChoice = strings.ToLower(approvChoice)
	}
	return charClass
}
...
func main() {
	...
	charClass := choosecCharClass()

	fmt.Println(start_training(	var charName string
, charClass))
}
